...
let ATOM = 100000000
let USD = 100000000
let DEFAULT = 100000000

let precision = { BTC, ETH, USD, LTC, GNT, ATOM, DEFAULT }
let debug = false
...
const _balance = async function (account, coin, redis) {
	...
	try {
		...
		if (!balance) return 0
		balance = parseInt(balance, 10) / precision['DEFAULT']
		return balance
	} catch (e) {
...
const _credit = async function (account, amount, coin, redis) {
	...
	try {
		...
		// prescision
		let atomicicity = Big(precision['DEFAULT'])
...
const _debit = async function (account, amount, coin, redis) {
	...
	try {
		...
		// atomic
		amount = parseInt(amount * precision['DEFAULT'], 10)
		...
		balance = parseInt(balance * precision['DEFAULT'], 10)
		log.debug(tag, coin + ' balance: ', balance)
		log.debug(tag, coin + ' amount: ', amount)
